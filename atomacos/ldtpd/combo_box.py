...
class ComboBox(Utils):
    def selectitem(self, window_name, object_name, item_name):
        ...
        try:
            menu_handle = self._internal_menu_handler(
                object_handle, menu_list, True
            )
            # Required for menuitem to appear in accessibility list
            self.wait(1)
            if not menu_handle.AXEnabled:
                raise LdtpServerException(
                    u"Object %s state disabled" % menu_list[-1]
                )
            menu_handle.Press()
    ...
    def selectindex(self, window_name, object_name, item_index):
        ...
        try:
            if item_index < 0 or item_index > length:
                raise LdtpServerException(
                    u"Invalid item index %d" % item_index
                )
    ...
    def verifyselect(self, window_name, object_name, item_name):
        ...
        try:
            ...
            if (
                re.match(item_name, title, re.M | re.U | re.L)
                or re.match(item_name, label, re.M | re.U | re.L)
                or re.match(
                    item_name, u"%u%u" % (role, label), re.M | re.U | re.L
                )
            ):
                return 1
        except LdtpServerException:
