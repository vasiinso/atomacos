...
class LdtpCustomLog(logging.Handler):
    ...
    def emit(self, record):
        ...
        self.log_events.append(
            u"%s-%s" % (record.levelname, record.getMessage())
        )
    ...
    def get_cpu_memory_stat(self):
        ...
        for p in psutil.process_iter():
            if self._stop:
                ...
                return proc_list
            if not re.match(
                fnmatch.translate(self._appname), p.name, re.U | re.L
            ):
...
class Utils(object):
    ...
    def _glob_match(self, pattern, string):
        ...
        return bool(
            re.match(fnmatch.translate(pattern), string, re.M | re.U | re.L)
        )
    ...
    def _get_windows(self, force_remap=False):
        ...
        for gui in set(self._running_apps):
            if (
                ...
                and self._app_under_test != gui.bundleIdentifier()
                and self._app_under_test != gui.localizedName()
            ):
    ...
    def _get_title(self, obj):
        ...
        try:
            ...
            if (
                re.match(
                    "(AXStaticText|AXRadioButton|AXButton)",
                    role,
                    re.M | re.U | re.L,
                )
                and (
                    desc == "text"
                    or desc == "radio button"
                    or desc == "button"
                )
                and obj.AXValue
            ):
        ...
        try:
            ...
            try:
                text = re.match(
                    "(AXTextField|AXTextArea)", role, re.M | re.U | re.L
                )
                if text:
                    title = obj.AXFilename
                else:
                    if not re.match("(AXTabGroup)", role, re.M | re.U | re.L):
                ...
                try:
                    if not re.match(
                        "(AXList|AXTable)", role, re.M | re.U | re.L
                    ):
                    ...
                    pass
        if not title:
            if re.match("(AXButton|AXCheckBox)", role, re.M | re.U | re.L):
                try:
                    title = obj.AXRoleDescription
                    if title:
                        return title
                    ...
                    pass
            elif re.match("(AXStaticText)", role, re.M | re.U | re.L):
                try:
                    title = obj.AXValue
                    if title:
                        return title
    ...
    def _singleclick(self, window_name, object_name):
        ...
        self.wait(0.5)
        self.generatemouseevent(
            size[0] + size[2] / 2, size[1] + size[3] / 2, "b1c"
        )
        return 1
    ...
    def _get_window_handle(self, window_name, wait_for_window=True):
        ...
        def _internal_get_window_handle(windows):
            ...
            for window in windows:
                ...
                if (
                    re.match(window_name, window)
                    or re.match(window_name, label)
                    or re.match(window_name, stripped_label)
                    or re.match(stripped_window_name, window)
                    or re.match(stripped_window_name, label)
                    or re.match(stripped_window_name, stripped_label)
                ):
        ...
        if not window_obj[0]:
            raise LdtpServerException(
                'Unable to find window "%s"' % orig_window_name
            )
        return window_obj

    def _get_object_handle(
        self,
        window_name,
        obj_name,
        obj_type=None,
        ...
        try:
            return self._internal_get_object_handle(
                window_name, obj_name, obj_type, wait_for_object, force_remap
            )
            ...
            return self._internal_get_object_handle(
                window_name, obj_name, obj_type, wait_for_object
            )

    def _internal_get_object_handle(
        self,
        window_name,
        obj_name,
        obj_type=None,
        ...
        try:
            ...
            # Call the method again, after updating apps
            obj = self._get_object_map(
                window_name, obj_name, obj_type, wait_for_object, True
            )
    ...
    def _get_object_map(
        ...
        window_handle, ldtp_window_name, app = self._get_window_handle(
            window_name, wait_for_object
        )
        ...
        object_list = self._get_appmap(
            window_handle, ldtp_window_name, force_remap
        )
        ...
        def _internal_get_object_handle(object_list):
            ...
            for obj in object_list:
                ...
                if (
                    re.match(obj_name, obj)
                    or re.match(obj_name, label)
                    or re.match(obj_name, stripped_label)
                    or re.match(stripped_obj_name, obj)
                    or re.match(stripped_obj_name, label)
                    or re.match(stripped_obj_name, stripped_label)
                ):
                    # Return object map
        ...
        for retry in range(0, obj_timeout):
            ...
            # Force remap
            object_list = self._get_appmap(
                window_handle, ldtp_window_name, True
            )
            # print(object_list)
    ...
    def _get_menu_handle(self, window_name, object_name, wait_for_window=True):
        window_handle, name, app = self._get_window_handle(
            window_name, wait_for_window
        )
        if not window_handle:
            raise LdtpServerException("Unable to find window %s" % window_name)
        ...
        for menu_handle in menu_handle_list:
            sub_menu_handle = self._get_sub_menu_handle(
                menu_handle, object_name
            )
            if sub_menu_handle:
    ...
    def _get_sub_menu_handle(self, children, menu):
        ...
        for current_menu in children.AXChildren:
            ...
            if (
                re.match(tmp_menu, label)
                or re.match(tmp_menu, u"%s%s" % (role, label))
                or re.match(stripped_menu, label)
                or re.match(stripped_menu, u"%s%s" % (role, label))
            ):
                return current_menu
        raise LdtpServerException("Unable to find menu %s" % menu)

    def _internal_menu_handler(
        self, menu_handle, menu_list, perform_action=False
    ):
